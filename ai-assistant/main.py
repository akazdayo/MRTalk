...
async def get_current_user(token: str) -> User:
    ...
    session = await prisma.session.find_unique(where={"token": token}, include={"user": True})
...
@app.get("/")
async def talk() -> Dict[str, str]:
    return {"message": "welcome to MRTalk API."}

@app.get("/chat")
async def talk(text: str, user_id: str, character_id: str, current_user: User = Depends(get_current_user)) -> Dict[str, str]:
    ...
    with PostgresStore.from_conn_string(
      ...
      response = await chat.ainvoke({"messages": [{"role": "user", "content": text}], "user_id": user_id, "character_id": character_id})
